...
export default function CotacaoLinkPage() {
  ...
  useEffect(() => {
    async function resolve() {
      ...
      const params = new URLSearchParams({ phone: data.phone, unidade: data.unidade });
      if (data.name) params.set('name', data.name);

      navigate(`/cotacao?${params.toString()}`, { replace: true });
    }
    resolve();
  }, [token, navigate]);
